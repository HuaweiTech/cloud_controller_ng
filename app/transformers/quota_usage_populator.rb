module VCAP::CloudController
  class QuotaUsagePopulator
    ...
    def space_routes_count(space)
      route_count = 0
      space.eager(:routes).all do |space_obj|
        route_count += space_obj.routes.count
      end
      route_count
    end
    ...
    def space_services_count(space)
      service_count = 0
      space.eager(:service_instances).all do |space_obj|
        service_count += space_obj.service_instances.count
      end
      service_count
    end
    ...
    def space_memory_usage(space)
      memory_usage = 0
      space.eager(:apps).all do |space_obj|
        space_obj.apps.each do |app|
